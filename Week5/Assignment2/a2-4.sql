...
RETURN NUMBER; 
FUNCTION Total_Cost(v_fname student.first_name%TYPE,v_lname student.last_name%TYPE)
RETURN NUMBER;
FUNCTION Total_Cost(v_zip student.zip%TYPE)
RETURN NUMBER;
END My_pack;
/
...
BEGIN
	...
	LEFT JOIN section se ON e.section_id = se.section_id
	LEFT JOIN course c ON se.course_no = c.course_no
	WHERE s.first_name = INITCAP(v_fname) AND s.last_name = INITCAP(v_lname);	
